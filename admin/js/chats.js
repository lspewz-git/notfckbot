...
import { $, setText, escapeHtml } from './dom.js';
...
function openDM(id, name) {
    ...
    $('confirm-dm').onclick = async () => {
        const message = $('dm-text').value.trim();
...
export async function submitAddContent() {
    ...
    if (isTv) body.notify_type = $('sub-notify-type').value;

    const ok = await mutate(
        isTv ? '/subscription' : '/watchlist',
        { method: 'POST', body },
        (res) => {
